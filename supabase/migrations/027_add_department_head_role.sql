-- Add department_head role for parent department supervisors.

ALTER TYPE public.user_role ADD VALUE IF NOT EXISTS 'department_head';

COMMENT ON COLUMN public.users.role IS 'admin | boss | department_head | manager | user';
...
BEGIN
  resolved_role := CASE
    WHEN meta_role IN ('admin', 'boss', 'department_head', 'manager', 'user')
