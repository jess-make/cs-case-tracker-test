...
function ResetPasswordButton({
  ...
  userId: string;
  disabled?: boolean;
  onReset: (result: { email: string; temporaryPassword: string }) => void;
}) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  ...
  function handleClick() {
    ...
    if (!window.confirm("確定要重設此使用者的密碼？將產生新的臨時密碼，使用者下次登入須修改密碼。")) {
      return;
    }
    setError(null);
    startTransition(async () => {
...
function UserDesktopTable({
  ...
  if (users.length === 0) {
    return (
      <div className="hidden rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-500 sm:p-12 md:block">
        目前沒有使用者
      </div>
    );
  }
...
export function UserManagementTable({
  ...
  function showTempPassword(
    result: { email: string; temporaryPassword: string },
    kind: "create" | "reset"
  ) {
    setTempPassword({
      email: result.email,
      temporaryPassword: result.temporaryPassword,
      title: kind === "create" ? "使用者已建立" : "密碼已重設",
      description:
        kind === "create"
          ? "請將以下臨時密碼提供給新使用者，首次登入後須修改密碼。"
          : "請將以下臨時密碼提供給使用者，下次登入後須修改密碼。",
    });
  }
    ...
    <>
      <div className="mb-4 flex justify-end">
        <button
          ...
          className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-700"
        >
      ...
      {users.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-500 sm:p-12 md:hidden">
          目前沒有使用者
        </div>
      ) : null}
      ...
      <UserMobileCards
        ...
        onReset={(result) => showTempPassword(result, "reset")}
      />
      <UserDesktopTable
        users={users}
        currentUserId={currentUserId}
