...
export async function getComplaintSourcesForManagement(): Promise<
  ComplaintSource[]
>;
export async function getComplaintSourcesForManagement(
...
export async function getComplaintSourcesForManagement(
  ...
  const { data, error } = await applyTaxonomySort(
    (await taxonomyClient(options)).from("complaint_sources").select("*")
  );
...
export async function getComplaintChannelsForManagement(): Promise<
  ComplaintChannel[]
>;
export async function getComplaintChannelsForManagement(
  options?: TaxonomyClientOptions
...
export async function getComplaintChannelsForManagement(
  ...
  const { data, error } = await applyChildTaxonomyManagementSort(
    (await taxonomyClient(options)).from("complaint_channels").select("*"),
    "source_id"
  );
...
export async function createComplaintChannel(
  ...
  if (error) {
    if (error.code === "23505") {
      throw new Error("此來源下已有相同服務管道名稱");
    }
    throw error;
  }
...
export async function renameComplaintChannel(
  ...
  if (duplicate) throw new Error("此來源下已有相同服務管道名稱");
  ...
  if (casesError) {
    throw new Error(`更新服務管道失敗：${casesError.message}`);
  }
  ...
  if (error) {
    ...
    if (error.code === "23505") throw new Error("此來源下已有相同服務管道名稱");
    throw new Error(`更新服務管道失敗：${error.message}`);
  }
...
export async function deleteComplaintChannel(id: string): Promise<void> {
  ...
  if (cases > 0) {
    throw new Error(
      `無法刪除服務管道，目前仍有 ${cases} 筆案件使用此服務管道。`
    );
  }
